  ...
  return (
    <Card className={`${getCardSize()} overflow-hidden hover:shadow-[var(--shadow-hover)] transition-all duration-300 cursor-pointer group bg-gradient-to-b from-card to-sport-light/30 border hover:border-sport-green/30`}>
      ...
      <div className="p-5 bg-gradient-to-t from-background to-transparent">
        ...
        <h3 className={`font-bold text-foreground mb-3 line-clamp-2 group-hover:text-sport-green transition-colors duration-300 ${
          size === 'large' ? 'text-xl leading-tight' : 'text-sm leading-snug'
        }`}>
          {news.title}
        </h3>
        ...
        {size === 'large' && (
          <div className="mt-4 flex items-center text-sport-green text-sm font-medium group-hover:translate-x-1 transition-transform duration-300">
            Lire la suite →
          </div>
        )}
      </div>
